...
partial class Tests {
  ...
  public async Task User_info_endpoint_request_with_token__access_user_informations__endpoint_receive_access_token () {
    ...
    var remoteClient = CreateHttpClient("http://oauth", "/userinfo", (request) => JsonContent.Create(new {
    ...
    var result = await AccessUserInfoAsync(authOptions, "token_abc", default!);
  ...
  public async Task Authentication_options_without_claims_issuer__access_user_informations__principal_with_scheme_name_as_issuer () {
    var context  = CreateHttpContext();
    var remoteClient = CreateHttpClient("http://oauth", "/userinfo", JsonContent.Create(new {test = "abc"}));
    var authOptions = CreateOAuthOptions(context, remoteClient);
    MapJsonClaim(authOptions, "test");

    var result = await AccessUserInfoAsync(authOptions, string.Empty, default!);
  ...
  public async Task Authentication_options_with_claims_issuer__access_user_informations__principal_with_options_issuer () {
    var context  = CreateHttpContext();
    var remoteClient = CreateHttpClient("http://oauth", "/userinfo", JsonContent.Create(new {test = "abc"}));
    var authOptions = CreateOAuthOptions(context, remoteClient) with { ClaimsIssuer = "issuer" };
    MapJsonClaim(authOptions, "test");
  ...
  public async Task User_info_endpoint_response_with_generic_error__access_user_informations__client_receive_error () {
    var context  = CreateHttpContext();
    var remoteClient = CreateHttpClient("http://oauth", "/userinfo", JsonContent.Create(new {message = "error"}), 400);
    var authOptions = CreateOAuthOptions(context, remoteClient);
    var result = await AccessUserInfoAsync(authOptions, string.Empty, default!);
