...
partial class OpenIdConnectFuncs
{
  ...
  const string UnexpectedToeknResponseContentType = "Unexpected token response format. Content-Type {0}. Status Code: {1}";
  ...
  const string IDX21339 = "The 'id_token' contains multiple audiences but 'azp' claim is missing.";
  ...
  static string? ValidateTokenResponse(HttpResponseMessage response)
  {
    ...
    if (IsEmptyString(contentType)) return FormatString(MissingContentTypeHeader, statusCode);
    if (!IsJsonContentTypeHttpResponse(contentType!)) return FormatString(UnexpectedToeknResponseContentType, contentType!, statusCode);
    return default;
  }

  public static string? ValidateTokenResponse(
    ...
    string clientId)
  {
    ...
    {
      if (!string.Equals(idToken.Payload.Azp, clientId))
    ...
    string code)
  {
    ...
    if ((cHashClaim as string) is null)
    ...
    string accessToken)
  {
    ...
    if ((atHashClaim as string) is null)
