...
partial class Tests {
  ...
  public async Task Access_token__access_user_informations__token_endpoint_receive_access_token () {
    ...
    var remoteClient = CreateHttpClient("http://oauth", "/userinfo", (request) => JsonContent.Create(new {query = request.RequestUri}));
    var authOptions = CreateFacebookOptions(new EphemeralDataProtectionProvider()) with { ClientSecret = "secret", RemoteClient = remoteClient, UserInformationEndpoint = "http://oauth/userinfo" };
    MapJsonClaim(authOptions, "query");

    var result = await AccessFacebookUserInfoAsync(authOptions, "token", default!);
  ...
  public async Task App_secret_proof__access_user_informations__token_endpoint_receive_app_secret_proof () {
    var context  = CreateHttpContext();
    var remoteClient = CreateHttpClient("http://oauth", "/userinfo", (request) => JsonContent.Create(new {secret_proof = request.RequestUri}));
    var authOptions = CreateFacebookOptions(new EphemeralDataProtectionProvider()) with { ClientSecret = "secret", RemoteClient = remoteClient, UserInformationEndpoint = "http://oauth/userinfo" };
    MapJsonClaim(authOptions, "secret_proof");

    var result = await AccessFacebookUserInfoAsync(authOptions, string.Empty, default!);
  ...
  public async Task Facebook_fields__access_user_informations__token_endpoint_receive_fields () {
    var context  = CreateHttpContext();
    var remoteClient = CreateHttpClient("http://oauth", "/userinfo", (request) => JsonContent.Create(new {fields = request.RequestUri}));
    var authOptions = CreateFacebookOptions(new EphemeralDataProtectionProvider()) with { ClientSecret = "secret", RemoteClient = remoteClient, UserInformationEndpoint = "http://oauth/userinfo" };
